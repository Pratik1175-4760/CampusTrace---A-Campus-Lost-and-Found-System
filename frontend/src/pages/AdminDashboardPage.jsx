...
const StatCard = ({ icon: Icon, label, value, sub, gradient, glow, index }) => (
  <div
    className="rounded-2xl p-5 fade-in relative overflow-hidden"
    style={{
      background: gradient,
  ...
  >
    ...
    />
    <div
      className="absolute -right-1 -top-1 w-12 h-12 rounded-full pointer-events-none"
    ...
    <div className="relative">
      ...
      <div className="text-4xl font-display font-bold text-white leading-none">{value ?? '—'}</div>
      {sub && <div className="text-xs text-white/60 mt-1.5">{sub}</div>}
    </div>
  </div>
)
  ...
  return (
    <div className="p-6 space-y-6 fade-in">
      ...
      <div>
        ...
        <p className="text-sm text-slate-500 mt-0.5">Overview of Lost & Found activity at PICT</p>
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-5 gap-4">
        ...
        {stats?.byCategory?.length > 0 && (
          ...
          >
            <h2 className="text-sm font-bold text-slate-700 mb-5">Items by Category</h2>
                ...
                return (
                  <div key={c._id}>
                    <div className="flex items-center justify-between mb-1">
                      <span className="text-sm text-slate-600 font-medium">{c._id}</span>
        ...
        >
          ...
          {loading
            ...
            : recent.map(item => (
              <div
                key={item._id}
                className="flex items-center gap-4 px-5 py-3.5 border-b border-slate-50 last:border-0 hover:bg-slate-50/80 transition-colors cursor-default"
              >
